...
export function registerTaxSummaryTool(server: McpServer, supabase: SupabaseClient, userId: string) {
  server.tool(
    'get_tax_summary',
    'Obtiene el resumen fiscal trimestral: IVA (Modelo 303), IRPF (Modelo 130), cuota de autónomo. Por defecto el trimestre actual.',
    {
      year: z.number().optional().describe('Año (por defecto el actual)'),
      quarter: z.number().min(1).max(4).optional().describe('Trimestre 1-4 (por defecto el actual)'),
    },
    async ({ year, quarter }) => {
